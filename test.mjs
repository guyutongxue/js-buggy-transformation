...
function checkParens(code) {
  ...
  if (
    /new\s+baz\s*\(/.test(code) ||
    /new\s*\(\s*baz\s*\(\s*\)\s*\)\s*\?\./.test(code)
  ) {
...
// ─── 8. bun ───
{
  ...
  const r = run(
    "npx bun build --target=bun src/input.ts --outfile=dist/bun/output.js 2>&1",
  );
  if (r.error) {
    results.push({ tool: label, error: r.error });
  } else {
...
// ─── 9. terser (parse-print round-trip, no compress/mangle) ───
{
  ...
  try {
    ...
    if (result.code) {
      ...
      results.push({
        tool: label,
        error: result.error?.message || "no output",
      });
    }
  } catch (e) {
    results.push({ tool: label, error: e.message });
